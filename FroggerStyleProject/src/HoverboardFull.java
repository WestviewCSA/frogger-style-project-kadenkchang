...
public class HoverboardFull{
	...
	public HoverboardFull() {
		forward = getImage("/imgs/"+"Hover.png"); //load the image for Tree
	...
	public boolean collided(Minion minion) {
		...
		Rectangle thisObject = new Rectangle (x+55,y,width-110,height-60);
		//checks if intersection
		return main.intersects(thisObject);
	}
	
	//2nd constructor - setting x and y
	public HoverboardFull(int x, int y) {
	...
	public void paint(Graphics g) {
		...
		//if(Frame.debugging) {
			...
			//g.drawRect(x+55, y, width-110, height-60);
	...
	private Image getImage(String path) {
		...
		try {
			URL imageURL = HoverboardFull.class.getResource(path);
